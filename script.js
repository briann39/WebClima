...
function searchWeather() {
    if (inputLocation.value === '') { // Verifica si el campo de entrada está vacío

        alert('Porfavor digita una ciudad');

    } else { // Si el campo no está vacío, procede a buscar el clima
        ...
        .then(data => { // Procesa la respuesta JSON de la API
            ...
            temperatureRec.textContent = `Max: ${Math.round(data.main.temp_max)} °C, Min: ${Math.round(data.main.temp_min)} °C`; // Mostrar temperatura máxima y mínima
            pronosticDisplay.textContent = `Pronostico: ${descriptionMap[data.weather[0].main]}`; // Mostrar el pronóstico del clima según el tipo de clima
            ...
            sourceBackground.src = backgroundMap[data.weather[0].main] || "backgrounds/default.mp4"; // Cambiar el video de fondo según el clima
            videoBackground.load(); // Cargar el nuevo video de fondo

            // Actualizar el icono del clima y su color
            iconDisplay.className = (iconMap[data.weather[0].main] || "fas fa-question-circle");
            iconDisplay.style.color = (data.weather[0].main === "Clear") ? "yellow" : "gray";
